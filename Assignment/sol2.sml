...
(* algorithms to compute prime numbers in the "Sieve of Eratosthenes" *)
(* 소수 구하는 에라토스테네스의 체 구현하는 알고리즘 *)

(* infinite list L = 2, 3, 4, 5에서 시작 *)
(* 2의 배수를 먼저 다 걸러내고 나면, 3의 배수, 5의 배수 ... 쭉쭉 걸러내는 방식 *)

(* primes() 함수 구현 *)

(* primes 함수가 sieve 함수 사용해야 하므로 먼저 구현 *)
(* sieve(lazyListVal) 함수 : 새로운 lazyList를 반환하는 함수 *)
...
fun seive(lazyListVal) = 
	case lazyListVal of
		nullList => nullList
        	| cons(hd, f) => cons(hd, fn() => seive(filterMultiples(cons(hd, f), hd)))



fun primes() =
	(* 위의 sieve를 let expression을 사용해서 할까 하다가 그냥 따로 빼서 구현함 *)
	(* #3-1에서 구현한 infSeq를 이용하여 2부터 시작하는 무한한 리스트를 생성함 *)
	seive(infSeq(2))
